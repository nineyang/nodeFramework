    ...
    parseMiddleware(middleware) {
        if (typeof middleware === 'string' && middleware.indexOf('@') > -1) {
            ...
            if (middleware.indexOf('#') > -1) {
                ...
                // 中间件
                let middlewarePath = require(pathObj.join(root, 'middlewares', middlewareParts.shift()));
                middleware = [new middlewarePath().handle];
            }

            controller = helper.withObj(pathObj.join(controllerPath , controller))[method];

            Array.isArray(middleware) ? middleware.push(controller) : middleware = controller;
